...
enum AppMain {
    static func main() {
        ...
        // Phase 1 (cross-platform core): .said bundle round-trip, the portability seams, the palette.
        if let idx = args.firstIndex(of: "--selftest-bundle") {
...
extension SelfTest {
    ...
    // MARK: - Phase 1 (cross-platform core) self-tests

    /// D2 — the `.said` bundle round-trips losslessly, the collision rule is deterministic, and an
    /// encrypted session exports to a bundle that opens WITHOUT the origin device's key.
    /// Temp dirs only; never touches ~/Desktop/Transcripts.
    static func runBundle(dir: String?) {
        setbuf(stdout, nil)
        print("== .said bundle self-test ==")
        var ok = true
        func check(_ l: String, _ c: Bool) { print("  \(c ? "✓" : "✗") \(l)"); ok = ok && c }

        let fm = FileManager.default
        let base = dir.map { URL(fileURLWithPath: $0) }
            ?? fm.temporaryDirectory.appendingPathComponent("said-bundle-selftest", isDirectory: true)
        try? fm.removeItem(at: base)
        try? fm.createDirectory(at: base, withIntermediateDirectories: true)
        defer { try? fm.removeItem(at: base) }

        // ---- Build a source session: segments, an images/ frame, and an audio.m4a.
        let srcRoot = base.appendingPathComponent("source", isDirectory: true)
        try? fm.createDirectory(at: srcRoot, withIntermediateDirectories: true)
        let sessionDir = srcRoot.appendingPathComponent("2026-08-13 09-00-00", isDirectory: true)
        let id = UUID()
        let segments = [
            TranscriptSegment(start: 0, end: 3, text: "Quorum intersection is the key property."),
            TranscriptSegment(start: 3, end: 7, text: "Any two majorities share at least one node."),
        ]
        var meta = SessionMeta(id: id, date: Date(timeIntervalSince1970: 1_786_000_000),
                               sourceLabel: "Mic", modelName: "openai_whisper-base.en",
                               title: "Consensus lecture", tags: ["distributed", "systems"],
                               audioFile: "audio.m4a", durationSeconds: 7,
        ...
        _ = try? AudioFileIO.writeCompactAudio(tone, to: sessionDir.appendingPathComponent("audio.m4a"))
        // An images/ frame with a relative path, to prove subdirectories survive.
        let images = sessionDir.appendingPathComponent("images", isDirectory: true)
        ...
        // ---- Export.
        let bundleURL = base.appendingPathComponent("lecture.said")
        do { _ = try SessionBundle.write(sessionDir: sessionDir, to: bundleURL) }
        catch { check("export threw: \(error)", false) }
        check("bundle file written", fm.fileExists(atPath: bundleURL.path))
        check("bundle is non-empty",
              ((try? fm.attributesOfItem(atPath: bundleURL.path)[.size] as? Int) ?? 0) > 0)

        // ---- Import into a FRESH root.
        let destRoot = base.appendingPathComponent("dest", isDirectory: true)
        try? fm.createDirectory(at: destRoot, withIntermediateDirectories: true)
        var imported: URL?
        do {
            let outcome = try SessionBundle.read(bundle: bundleURL, into: destRoot)
            check("first import reports .imported", !outcome.isDuplicate)
            imported = outcome.directory
        ...
        if let dst = imported {
            ...
            check("date matches", m?.date == originalMeta?.date)
            check("title matches", m?.title == originalMeta?.title)
            check("tags match", m?.tags == originalMeta?.tags)
            check("sourceLabel matches", m?.sourceLabel == originalMeta?.sourceLabel)
            check("bookmarks match", m?.bookmarks.map(\.time) == originalMeta?.bookmarks.map(\.time))
            check("speakerCount matches", m?.speakerCount == originalMeta?.speakerCount)
            check("speakerNames match", m?.speakerNames == originalMeta?.speakerNames)
            check("audioFile recorded", m?.audioFile == "audio.m4a")
            check("audio.m4a present", fm.fileExists(atPath: dst.appendingPathComponent("audio.m4a").path))
            ...
            // ---- Collision rule: re-import into the SAME root.
            let before = SessionStore.sessionDirectoryURLs(root: destRoot).count
            if let again = try? SessionBundle.read(bundle: bundleURL, into: destRoot) {
                check("re-import reports .alreadyPresent", again.isDuplicate)
                // Compare by folder NAME: /tmp→/private/tmp symlink resolution and trailing-slash
                // differences make full-URL equality unreliable here (same trap as --selftest-index).
                check("re-import points at the existing folder",
                      again.directory.lastPathComponent == dst.lastPathComponent)
            } else {
                check("re-import did not throw", false)
            }
            check("no duplicate folder created",
                  SessionStore.sessionDirectoryURLs(root: destRoot).count == before)
        }

        // ---- Encrypted source → bundle must open WITHOUT that key.
        SessionIO.overrideKey = SymmetricKey(size: .bits256)
        SessionIO.isEncryptionEnabled = true
        let encDir = srcRoot.appendingPathComponent("2026-08-13 10-00-00", isDirectory: true)
        let encID = UUID()
        ...
        catch { check("encrypted export threw: \(error)", false) }

        // Drop the key entirely — this is the "another Mac / another account" case.
        ...
        if let outcome = try? SessionBundle.read(bundle: encBundle, into: encRoot) {
            ...
            check("…and it kept its session id", DocumentBuilder.readSession(outcome.directory)?.meta.id == encID)
        } else {
            check("encrypted bundle import did not throw", false)
        }

        SessionIO.isEncryptionEnabled = false; SessionIO.overrideKey = nil
        print(ok ? "OK" : "FAIL"); exit(ok ? 0 : 2)
    }

    /// C1/C2 — the portability seams behave as specified on macOS, and SaidKit owns its resources.
    static func runPortability() {
        setbuf(stdout, nil)
        print("== portability self-test ==")
        var ok = true
        func check(_ l: String, _ c: Bool) { print("  \(c ? "✓" : "✗") \(l)"); ok = ok && c }

        // --- C1: session root provider.
        ...
        check("default root is ~/Desktop/Transcripts",
              SessionLocation.root.standardizedFileURL == expected.standardizedFileURL)
        check("SessionStore.root goes through the provider",
              SessionStore.root.standardizedFileURL == expected.standardizedFileURL)

        let injected = FileManager.default.temporaryDirectory
            .appendingPathComponent("said-portability-root", isDirectory: true)
        SessionLocation.rootProvider = { injected }
        check("injected root is honoured", SessionStore.root.path == injected.path)
        check("DocumentBuilder.makeSessionFolder follows the provider",
              DocumentBuilder.makeSessionFolder(date: Date(timeIntervalSince1970: 0))
                  .deletingLastPathComponent().path == injected.path)
        SessionLocation.resetRootToPlatformDefault()
        check("reset restores the platform default",
              SessionLocation.root.standardizedFileURL == expected.standardizedFileURL)
        try? FileManager.default.removeItem(at: injected)

        // --- C2: the trash seam. In this CLI binary nothing has injected (AppModel.onLaunch never
        // ran), so the macOS default MUST refuse rather than silently hard-delete.
        SessionTrash.resetToPlatformDefault()
        check("no Trash implementation injected in the CLI binary", !SessionTrash.isInjected)
        let victim = FileManager.default.temporaryDirectory
            .appendingPathComponent("said-portability-victim-\(UUID().uuidString)")
        try? Data("do not delete me".utf8).write(to: victim)
        var refused = false
        do { try SessionTrash.trash(victim) } catch { refused = true }
        check("un-injected macOS delete REFUSES", refused)
        check("…and the file still exists (no hard delete)",
              FileManager.default.fileExists(atPath: victim.path))

        var injectedTarget: URL?
        SessionTrash.inject { url in injectedTarget = url }
        check("inject() marks the seam injected", SessionTrash.isInjected)
        try? SessionTrash.trash(victim)
        check("injected implementation receives the URL", injectedTarget?.path == victim.path)
        try? FileManager.default.removeItem(at: victim)
        SessionTrash.resetToPlatformDefault()

        // --- D1: SessionMeta.id must be ADDITIVE. A session.json written before the field existed
        // must decode cleanly, and must NOT gain an "id" key merely by being read and re-written.
        // Only the explicit backfill may add it, and it must be stable once added.
        let schemaRoot = FileManager.default.temporaryDirectory
            .appendingPathComponent("said-schema-\(UUID().uuidString)", isDirectory: true)
        try? FileManager.default.createDirectory(at: schemaRoot, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: schemaRoot) }
        let legacyDir = schemaRoot.appendingPathComponent("2026-01-01 00-00-00", isDirectory: true)
        try? FileManager.default.createDirectory(at: legacyDir, withIntermediateDirectories: true)
        // Hand-written, pre-Phase-1 shape: no "id" key anywhere.
        let legacyJSON = """
        {"meta":{"date":694224000,"sourceLabel":"Mic","modelName":"openai_whisper-base.en",\
        "tags":[],"schemaVersion":2,"bookmarks":[],"chapters":[],"actionItems":[],\
        "summaries":{},"imported":false},"segments":[{"start":0,"end":2,"text":"legacy session"}]}
        """
        try? Data(legacyJSON.utf8).write(to: legacyDir.appendingPathComponent("session.json"))
        try? Data("[00:00] legacy session\n".utf8).write(to: legacyDir.appendingPathComponent("transcript.md"))

        let legacyDoc = DocumentBuilder.readSession(legacyDir)
        check("a pre-Phase-1 session.json still decodes", legacyDoc != nil)
        check("…and decodes with id == nil", legacyDoc?.meta.id == nil)
        if let legacyDoc {
            DocumentBuilder.writeSessionJSON(legacyDoc, to: legacyDir)   // read → write, no backfill
            let rewritten = (try? String(contentsOf: legacyDir.appendingPathComponent("session.json"), encoding: .utf8)) ?? ""
            check("…and a read→write round trip adds NO \"id\" key", !rewritten.contains("\"id\""))
        }
        let minted = SessionStore.ensureSessionID(dir: legacyDir)
        check("ensureSessionID backfills an id", minted != nil)
        let afterBackfill = (try? String(contentsOf: legacyDir.appendingPathComponent("session.json"), encoding: .utf8)) ?? ""
        check("…which is now present on disk", afterBackfill.contains("\"id\""))
        check("…and a second call is a stable no-op", SessionStore.ensureSessionID(dir: legacyDir) == minted)

        // --- Resource bundle: Packs/*.json must resolve from SaidKit's bundle.
        let packs = PackManager.shared.reload()
        check("Bundle.module resolves Packs/*.json from SaidKit", packs.count >= 4)
        check("every pack has an id and a name", packs.allSatisfy { !$0.id.isEmpty && !$0.name.isEmpty })
    ...
    static func runTheme() {
        ...
        /// Resolve a SwiftUI Color to concrete RGBA under a given appearance. Returns nil when the
        /// token cannot be realized at all — which is exactly what this test is looking for.
        func rgba(_ c: Color, _ appearance: NSAppearance) -> (CGFloat, CGFloat, CGFloat, CGFloat)? {
        ...
        let tokens: [(String, Color)] = [
            ("windowBG", Theme.windowBG), ("titlebar", Theme.titlebar), ("sidebarBG", Theme.sidebarBG),
            ("inspectorBG", Theme.inspectorBG), ("surface", Theme.surface), ("surface2", Theme.surface2),
            ("statusBG", Theme.statusBG), ("rowHover", Theme.rowHover),
            ("hairline", Theme.hairline), ("hairline2", Theme.hairline2),
            ("text", Theme.text), ("text2", Theme.text2), ("text3", Theme.text3),
            ("textSidebar", Theme.textSidebar),
            ("selection", Theme.selection), ("onSelection", Theme.onSelection),
            ("selectionSoft", Theme.selectionSoft), ("playingBand", Theme.playingBand),
            ("highlight", Theme.highlight), ("highlightActive", Theme.highlightActive),
            ("record", Theme.record), ("recordText", Theme.recordText),
            ("recordSoft", Theme.recordSoft), ("recordBorder", Theme.recordBorder),
            ("pause", Theme.pause), ("pauseText", Theme.pauseText),
            ("pauseSoft", Theme.pauseSoft), ("pauseBorder", Theme.pauseBorder),
            ("accent", Theme.accent), ("accentSoft", Theme.accentSoft),
            ("accentBorder", Theme.accentBorder), ("accentText", Theme.accentText),
            ("aiTint", Theme.aiTint),
            ("ok", Theme.ok), ("okText", Theme.okText), ("okSoft", Theme.okSoft), ("okBorder", Theme.okBorder),
        ]
        var unresolved: [String] = []
        for (name, color) in tokens where rgba(color, light) == nil || rgba(color, dark) == nil {
            unresolved.append(name)
        }
        check("all \(tokens.count) tokens resolve in BOTH appearances (\(unresolved.joined(separator: ", ")))",
              unresolved.isEmpty)

        for (label, appearance) in [("light", light), ("dark", dark)] {
            var seen: [String: Int] = [:]
            var collisions: [String] = []
            for slot in 1...8 {
                guard let c = rgba(Theme.speakerColor(slot), appearance) else {
                    collisions.append("slot \(slot) unresolved"); continue
                }
                let key = String(format: "%.3f,%.3f,%.3f", c.0, c.1, c.2)
                if let prior = seen[key] { collisions.append("slot \(slot) == slot \(prior)") }
                seen[key] = slot
            }
            check("8 speaker slots are distinct in \(label) (\(collisions.joined(separator: "; ")))",
                  collisions.isEmpty)
        }

        let one = rgba(Theme.speakerColor(1), light)
        let nine = rgba(Theme.speakerColor(9), light)
        check("slot 9 cycles back to slot 1",
              one != nil && nine != nil && one! == nine!)
        check("slot 0 / negative slots are clamped, not a crash",
              rgba(Theme.speakerColor(0), light) != nil && rgba(Theme.speakerColor(-3), light) != nil)
