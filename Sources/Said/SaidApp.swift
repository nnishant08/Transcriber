...
/// Appends to `<temp>/transcriber_launch.log`. Used only to verify the launch path during bring-up.
/// The FILENAME deliberately keeps its old name (it is developer-facing, and muscle memory /
/// existing notes point at it); only the hardcoded `/tmp` path is gone.
func debugLog(_ s: String) {
    let line = "\(Date()): \(s)\n"
    let url = FileManager.default.temporaryDirectory.appendingPathComponent("transcriber_launch.log")
...
struct SaidApp: App {
    ...
    var body: some Scene {
        ...
        .commands { SaidCommands(model: model) }
    }
}
    ...
    func showMain(_ route: ShellRoute) {
        ...
        if mainWindow == nil {
            mainWindow = makeWindow(
                ...
                customTitlebar: true,
                // NOT renamed with the rebrand, deliberately: an NSWindow autosave name IS a
                // UserDefaults key ("NSWindow Frame TranscriberMainWindow"). Renaming it would
                // silently discard every user's saved window position and size. §E1 keeps
                // UserDefaults keys stable; this is one of them.
