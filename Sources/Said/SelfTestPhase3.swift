...
extension SelfTest {
    ...
    static func runEdit() {
        ...
        let mdURL = SessionPaths.transcriptURL(in: dir)   // session-named, never a hardcoded transcript.md
        let mdBefore = try? Data(contentsOf: mdURL)
        try? EditStore.write([e1], dir: dir)
        let mdAfter = try? Data(contentsOf: mdURL)
        c.check("the transcript exists on disk", mdBefore != nil)
        c.check("transcript is byte-identical after an edit", mdBefore == mdAfter)
