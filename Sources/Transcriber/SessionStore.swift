...
enum SessionStore {
    ...
    static var root: URL { AppModel.transcriptsDirectory }

    // MARK: - Listing

    /// All session folders under `root` (a directory containing a `transcript.md`), as URLs.
    static func sessionDirectoryURLs(root: URL = SessionStore.root) -> [URL] {
    ...
    static func synthMeta(dir: URL) -> SessionMeta {
    ...
    static func stripLeadingSpeakerLabel(_ s: String) -> String {
    ...
    static func timestampedTranscript(dir: URL, maxChars: Int = 12_000) -> String {
        guard let raw = SessionIO.readText(dir.appendingPathComponent("transcript.md")) else { return "" }
        var lines = raw.components(separatedBy: "\n")
        if let sep = lines.firstIndex(where: { $0.trimmingCharacters(in: .whitespaces) == "---" }) {
            lines.removeFirst(sep + 1)
    ...
    static func timedSegments(dir: URL) -> [TranscriptSegment] {
    ...
    static func snippet(plainText text: String, max: Int = 160) -> String {
    ...
    static func stripLeadingTimestamp(_ s: String) -> String {
    ...
    static func ensureTitle(dir: URL, force: Bool = false) async {
    ...
    @discardableResult
    static func backfillTags(dir: URL, force: Bool = false) async -> [String] {
        guard var doc = DocumentBuilder.readSession(dir) else { return [] }
    ...
    static func postSessionSaved(_ dir: URL) {
    ...
    @discardableResult
    static func migrateLegacyFlatFiles(root: URL = SessionStore.root) -> MigrationResult {
        let fm = FileManager.default
...
actor TitleBackfill {
    static let shared = TitleBackfill()
