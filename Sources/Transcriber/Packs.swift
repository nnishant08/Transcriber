...
struct Pack: Codable, Identifiable, Sendable, Hashable {
    var id: String
    var name: String
...
final class PackManager: @unchecked Sendable {
    ...
    @discardableResult
    func reload(from directory: URL? = nil) -> [Pack] {
    ...
    var availablePacks: [Pack] { packs.filter { Entitlements.isEntitled(.verticalPack($0.id)) } }
    ...
    func mergedVocabulary(userVocab: [String]) -> [String] {
        var seen = Set<String>()
        var out: [String] = []
