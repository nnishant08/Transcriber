...
public enum ClipExportError: LocalizedError {
...
public struct ClipResult: Sendable { public var url: URL; public var duration: Double }
...
public enum ClipExporter {
    ...
    @discardableResult
    public static func exportClip(audioURL: URL, start: Double, end: Double, to output: URL) async throws -> ClipResult {
    ...
    public static func renderWaveform(samples: [Float], size: CGSize = CGSize(width: 1280, height: 720),
    ...
    /// Burn the caption into the waveform frame.
    ///
    /// CoreText, not AppKit. The original used `NSFont` / `NSColor` / `NSGraphicsContext`, which is
    /// what kept this file macOS-only. CoreText draws identically on both platforms against the same
    /// `CGContext`, so this needs no `#if` seam at all — it is genuinely one implementation.
    /// Geometry, font size, weight, colour, centring and tail-truncation are unchanged.
    private static func drawCaption(_ text: String, in ctx: CGContext, size: CGSize) {
        ...
        let rect = CGRect(x: inset, y: size.height * 0.06,
                          width: size.width - inset * 2, height: size.height * 0.26)

        let font = CTFontCreateUIFontForLanguage(.system, size.height * 0.045, nil)
            ?? CTFontCreateWithName("Helvetica" as CFString, size.height * 0.045, nil)

        // Centred, truncating tail — the CTParagraphStyle equivalent of the old NSParagraphStyle.
        var alignment = CTTextAlignment.center
        var lineBreak = CTLineBreakMode.byTruncatingTail
        let settings: [CTParagraphStyleSetting] = [
            ...
            CTParagraphStyleSetting(spec: .lineBreakMode,
                                    valueSize: MemoryLayout<CTLineBreakMode>.size, value: &lineBreak),
        ]
        let paragraph = CTParagraphStyleCreate(settings, settings.count)

        let attributed = NSAttributedString(string: String(text.prefix(240)), attributes: [
            kCTFontAttributeName as NSAttributedString.Key: font,
            kCTForegroundColorAttributeName as NSAttributedString.Key: CGColor(gray: 0.95, alpha: 1),
        ...
        ctx.saveGState()
        // CoreText lays out bottom-up in the context's own coordinate space, matching the
        // unflipped NSGraphicsContext the old code created.
    ...
    @discardableResult
    public static func exportAudiogram(audioURL: URL, start: Double, end: Double,
