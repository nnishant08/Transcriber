...
public enum SessionIO {
    ...
    public static var requireTouchID: Bool {
    ...
    public static var overrideKey: SymmetricKey?
    ...
    public static func isEncryptedBlob(_ data: Data) -> Bool { data.count > magic.count && data.prefix(magic.count) == magic }
    ...
    public static func decryptIfNeeded(_ data: Data) -> Data {
    ...
    private static func storeKeychainKey(_ key: SymmetricKey) -> Bool {
        ...
        let add: [String: Any] = [
            ...
            // ThisDeviceOnly: this is a PER-INSTALL key, so it must never ride an encrypted backup
            // to another device — a key that migrates is a key that outlives the data it protects.
            // (The previous value, plain `AfterFirstUnlock`, was strictly more permissive.) No access
            // group is set, so the item stays private to this app and the query works in an iOS
            // sandbox unchanged. Existing keys keep decrypting: the read query does not constrain
            // accessibility, so only newly written keys pick up the tighter class.
    ...
    /// The default prompt shown by the biometric gate. `LAPolicy.deviceOwnerAuthenticationWithBiometrics`
    /// is cross-platform; only the wording differs, because the user is looking at a different sensor.
    ///
    /// PHASE 2 (iOS): Face ID additionally requires an `NSFaceIDUsageDescription` entry in the iOS
    /// app's Info.plist, or `evaluatePolicy` fails outright. That plist belongs to the iOS app target,
    /// which does not exist yet — it is deliberately not added here.
    public static var defaultUnlockReason: String {
        #if os(macOS)
        return "Unlock your encrypted transcripts"
        #else
        return "Unlock your encrypted sessions"
        #endif
    }

    /// Prompt for Touch ID / Face ID when `requireTouchID` is on. Returns true if not required, or on
    /// success. A failure returns false so the caller can keep data locked — never silently exposes
    /// plaintext.
    @discardableResult
    static func authenticateIfNeeded(reason: String = SessionIO.defaultUnlockReason) async -> Bool {
    ...
    public static func enableEncryption(root: URL = SessionStore.root, backup: Bool = true) throws {
    ...
    public static func disableEncryption(root: URL = SessionStore.root) throws {
        guard let key = currentKey() else { throw SessionIOError.noKey }
        for dir in SessionStore.sessionDirectoryURLs(root: root) {
            for url in contentFiles(in: dir) {
                let raw = try Data(contentsOf: url)
