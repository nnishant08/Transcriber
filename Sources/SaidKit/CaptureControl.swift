...
public enum AudioActivity {
    ...
    public static let silenceRMS: Float = 0.004
    /// Default idle time before an automatic pause.
    public static let defaultAutoPauseSeconds: TimeInterval = 30
    ...
    public static let prerollSeconds: Double = 1.0
    ...
    public static let stallSeconds: TimeInterval = 3.0
    /// Minimum spacing between two recovery attempts for the same source.
    public static let recoveryCooldown: TimeInterval = 6.0
}
...
public final class CaptureGate: SampleReceiver, @unchecked Sendable {
    ...
    public init(downstream: any SampleReceiver, prerollSeconds: Double = AudioActivity.prerollSeconds) {
    ...
    public func open(flushPreroll: Bool = true) {
        lock.lock()
    ...
    public var level: Float { lock.lock(); defer { lock.unlock() }; return lastRMS }
    /// When this gate last received ANY samples (nil = never). Drives the stall watchdog.
    public var lastDeliveryAt: TimeInterval? { lock.lock(); defer { lock.unlock() }; return lastDelivery }
    public var prerollCount: Int { lock.lock(); defer { lock.unlock() }; return preroll.count }
...
public enum CaptureDecision: Equatable { case none, autoPause, autoResume }
...
public struct SilenceMonitor {
    ...
    public var pauseAfter: TimeInterval = AudioActivity.defaultAutoPauseSeconds
    ...
    public init(enabled: Bool = true, pauseAfter: TimeInterval = AudioActivity.defaultAutoPauseSeconds) {
    ...
    public mutating func update(level: Float, now: TimeInterval, paused: Bool, reason: PauseReason?) -> CaptureDecision {
        if level >= threshold {
    ...
    public func quietSeconds(now: TimeInterval) -> TimeInterval {
...
public struct StallMonitor {
    public var stallAfter: TimeInterval = AudioActivity.stallSeconds
    public var cooldown: TimeInterval = AudioActivity.recoveryCooldown

    public init(stallAfter: TimeInterval = AudioActivity.stallSeconds,
                cooldown: TimeInterval = AudioActivity.recoveryCooldown) {
    ...
    public mutating func start(now: TimeInterval) { lastActionAt = now }

    public mutating func shouldRecover(lastDelivery: TimeInterval?, now: TimeInterval) -> Bool {
...
public struct SessionClock {
    ...
    public init(t0: TimeInterval) { self.t0 = t0 }

    var isPaused: Bool { pausedAt != nil }

    public mutating func pause(now: TimeInterval) { if pausedAt == nil { pausedAt = now } }
