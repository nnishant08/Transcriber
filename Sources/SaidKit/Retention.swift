...
public struct RetentionPolicy: Codable, Sendable, Equatable {
    public var autoDeleteEnabled: Bool = false
    public var maxAgeDays: Int = 30
    public var deleteAudioOnly: Bool = false   // when true, the sweep removes only audio (keeps the transcript)

    public init(autoDeleteEnabled: Bool = false, maxAgeDays: Int = 30, deleteAudioOnly: Bool = false) {
        self.autoDeleteEnabled = autoDeleteEnabled
        self.maxAgeDays = maxAgeDays
        self.deleteAudioOnly = deleteAudioOnly
    }
...
public struct RetentionResult: Sendable {
    ...
    public var skippedLocked: Int = 0
}
...
public enum Retention {
    ...
    /// Destroy a URL (synchronous, throwing). Routes through the `SessionTrash` seam (C2): on macOS
    /// `Said` injects `FileManager.trashItem` at launch, on iOS the SaidKit default removes directly,
    /// and an un-injected macOS path REFUSES rather than hard-deleting. Overridden in tests.
    public static func trashToSystem(_ url: URL) throws {
        try SessionTrash.trash(url)
    }
    ...
    @discardableResult
    public static func sweep(root: URL = SessionStore.root,
    ...
    @discardableResult
    public static func deleteAllAudio(root: URL = SessionStore.root,
    ...
    @discardableResult
    public static func deleteOlderThan(days: Int, root: URL = SessionStore.root, now: Date = Date(),
                                trash: (URL) throws -> Void = Retention.trashToSystem) -> Int {
